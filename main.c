...
int _fill_buffer(FILE *file_p) {
  ...
  file_p->counter =
      read(file_p->file_descriptor, file_p->next_char_pos_p, buffer_size);
...
FILE *file_open(char *name, char *mode) {
  ...
  for (file_p = _io_buffer; file_p < _io_buffer + MAX_NR_OF_OPEN_FILES;
       ++file_p) {
...
// NOTE: Using bit fields leads to more verbose syntax. However, it's more
// readable.
